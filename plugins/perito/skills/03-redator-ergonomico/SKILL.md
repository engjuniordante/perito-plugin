...
`python3 scripts/build_laudo_ergo.py "<00-Template/template-ergonomico.docx>" laudo-data-ergo.json <planilha-do-caso.xlsx> "<config.caminhos.saida_laudos>/laudo-ergonomico-<processo>.docx"`
...
- **SAÍDA = `config.caminhos.saida_laudos`** (no Irineu: `Base Perícia Irineu/Laudos-Gerados/`) — o **mesmo destino da 02 e da 04/04b**, para os laudos não se espalharem. O script **cria a pasta** se ela não existir.
...
⚠ Nunca no Desktop (o sandbox do Cowork não acessa).
