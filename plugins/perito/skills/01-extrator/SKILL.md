...
**⛔ Passo final OBRIGATÓRIO — guard determinístico de EPI:** depois de gravar o `.md`, rodar `python3 <pasta desta skill>/check_epi.py <arquivo gravado> <base_conhecimento>/04-EPIs/CA-dicionario.json` (o caminho da base sai do `perito-config.json`). O script edita o próprio formulário: **(1) LOOKUP por C.A.** no `CA-dicionario.json` — se o C.A. consta, reescreve o agente pelo valor do dicionário, **ignorando o nome do produto**; **(2) REGRA ABSOLUTA** (C.A. fora do dicionário): creme/pomada → `Químico dérmico (An.13)`, **exceto "protetor solar"**; **(3) crava o bloco `🚩 VERIFICAÇÃO AUTOMÁTICA DE EPI`** com o que classificou (🔧), o que só o C.A. resolve (🚩) e os **C.A. NÃO CATALOGADOS** (📇). Não depende de você obedecer regra em prosa — é a rede que impede a cagada de sair no formulário. Reproduzir o resultado do script na resposta ao perito (não reabrir o `.md` — o script já editou). Saiu `🚩` ou `📇` → avisar o perito (classificação a confirmar pelo C.A. / C.A. a catalogar antes de fechar o laudo).
