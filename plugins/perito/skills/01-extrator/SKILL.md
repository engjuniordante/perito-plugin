...
**⛔ Passo final OBRIGATÓRIO — guard determinístico de EPI:** depois de gravar o `.md`, rodar `python3 <pasta desta skill>/check_epi.py <arquivo gravado>`. O script varre as classificações de EPI e **crava um bloco `🚩 VERIFICAÇÃO AUTOMÁTICA DE EPI` dentro do próprio formulário** quando acha classificação suspeita (creme em radiação/UV, EPI em An.7 sem ser máscara/lente de solda, capa como químico…). Não depende de você obedecer regra em prosa — é a rede que impede a cagada de sair no formulário. Reproduzir o resultado do script na resposta ao perito (não reabrir o `.md` para conferir — o script já editou). Saiu `🚩` → avisar o perito que há classificação de EPI a confirmar pelo C.A.
