...
**⛔ Passo final OBRIGATÓRIO — guard determinístico de EPI:** depois de gravar o `.md`, rodar `python3 <pasta desta skill>/check_epi.py <arquivo gravado>`. O script edita o próprio formulário: **(1) AUTO-CORRIGE** creme/pomada para `Químico dérmico (An.13)` (regra absoluta) direto na linha de EPI; **(2) crava o bloco `🚩 VERIFICAÇÃO AUTOMÁTICA DE EPI`** listando o que corrigiu (🔧) e o que **só o C.A. resolve** (🚩: EPI em radiação/UV sem ser máscara/lente de solda, capa como químico…). Não depende de você obedecer regra em prosa — é a rede que impede a cagada de sair no formulário. Reproduzir o resultado do script na resposta ao perito (não reabrir o `.md` — o script já editou). Saiu `🚩` → avisar que há classificação de EPI a confirmar pelo C.A.
...
1. **Classifique pela FUNÇÃO do EPI (o que o C.A. aprova), NUNCA pelo nome comercial / linha / fantasia.** "G3", "Luz Negra", "Protek", "Solar", etc. são **marca/linha — não são agente**. **NUNCA inferir o anexo a partir do nome do produto** (ex.: "Luz Negra" **não** é UV/radiação). Se você só tem o nome e não a função, isso é palpite → trate como palpite (regra 3).
2. **Creme protetor / creme de proteção = SEMPRE químico dérmico (An.13).** É barreira dérmica contra óleos/graxas/solventes/álcalis. **NUNCA** classificar creme em RNI/radiação (An.7) nem em qualquer outro anexo — não existe "creme" que tire o creme do An.13.
3. **Classificação é SUGESTÃO, não veredicto.** Quando o agente não vier de descrição funcional explícita (só do nome) → marcar `[agente a confirmar pelo C.A.]`. O perito confirma pelo C.A. in loco.
4. **DIREÇÃO PROIBIDA = EXCLUSÃO.** Uma classificação **nunca** pode ser usada para dizer que o EPI **não** protege um agente, nem para afastar/elidir agente. Errar **incluindo** (+ flag) é tolerável; errar **excluindo** corrompe o laudo inteiro. Na dúvida → vincule ao agente químico mais provável **+ flag**, jamais remova.
