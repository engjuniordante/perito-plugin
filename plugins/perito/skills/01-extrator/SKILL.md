...
- **BANHEIROS DE GRANDE CIRCULAÇÃO (item 3 / Anexo 14):** quando a Inicial alegar **higienização/limpeza de sanitários de uso coletivo ou de grande circulação** (banheiros públicos, hospitalares, de refeitório etc.), marque o checkbox `[X] Limpeza de banheiros de grande circulação` na seção **M. AGENTES BIOLÓGICOS** e deixe o bloco **▶ BANHEIROS DE GRANDE CIRCULAÇÃO** com os `____` intactos + no Obs do An.14 lembre "medir in loco: nº de banheiros/vasos por local e rotatividade diária (alimenta a tabela do item 3)". **NUNCA preencher números** — vêm da medição in loco do perito; bloco em branco → o redator não emite a tabela. Alegação só na Inicial e negada na Contestação: marque assim mesmo (é o gatilho de coleta), a caracterização é decidida in loco.
...
**QUESITOS (Parte 4) — DECISÃO POR BLOCO (não quesito a quesito):** não confiar no rótulo do arquivo — autoria pela **intenção**: Reclamante **constrói** (exige medição, exposição, falha de EPI); Reclamada **desmonta/presume EPI eficaz** (tom cético, ônus invertido, timbre de assistente técnico). Conflito → vale a intenção; dúvida → "[autoria provável: ___ — confirmar]". Bloco **SÓ médico** (lesão, nexo, incapacidade, CID) → só a linha "Bloco de perícia médica — não pertinente ao perito técnico." ⚠ **Ergonomia (NR-17), segurança de máquinas (NR-12) e reconstituição de acidente SÃO do perito de engenharia → transcrever INTEIRO**, nunca descartar pelo rótulo do capítulo. Bloco **técnico ou genérico** → transcrever **INTEIRO**, numeração original. ⚠ O extrator **transcreve, não tria** — na dúvida, transcreva. **Reclamada com dois blocos** (insalubridade + médico) → trazer o de insalubridade inteiro; do médico, só a linha-resumo. "Não localizado no PJE" quando for o caso.
