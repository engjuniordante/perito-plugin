...
3. **(Modo B) O CLI `nlm` tem de estar utilizável — confira ANTES de criar qualquer coisa.** Rode `nlm --version` (≥ `0.9.4`, ver o item 2). ⚠️ **`command not found` no comando cru NÃO significa CLI quebrado** — é o PATH quirk conhecido: o `nlm.exe` é instalado em `%APPDATA%\Python\Python3xx\Scripts\` e essa pasta costuma ficar fora do PATH do shell. Antes de declarar defeito, **tente o caminho completo** — `"$env:APPDATA\Python\Python312\Scripts
lm.exe" --version` (312/311/313/310) —, que é exatamente a busca que o `achar_nlm()` do script faz e o motivo de a extração rodar mesmo com o comando cru falhando. Só se **nenhum** dos dois responder, **PARE** e mande consertar (upgrade + `nlm login` + reiniciar o Code). ⛔ **Não escorregue para o caminho MCP na mão** — leia o *Fallback* no fim do Passo 1B antes de sequer considerá-lo: na mão a Parte 3a **não roda**, e o que sai de lá é uma ficha de EPI plausível e errada.
...
### Fallback (script indisponível) — MCP passo a passo, e quando ele NÃO serve

> ⛔ **Com ficha de EPI em arquivo separado, o caminho na mão está PROIBIDO — pare e conserte o `nlm`.** Não é preferência, é medição: (a) na mão os PDFs vão todos para um notebook só, e a 3a dividindo indexação com a contestação devolve **2 a 3 entregas de uma ficha que tem centenas**; (b) na mão não há **artefato**, então a 3a vai por chat, que **tem teto de resposta**; (c) o prompt da 3a é o maior do arquivo e passa raspando o **limite de ~4,8k chars por query** — acima dele a pergunta volta `INVALID_ARGUMENT` e a 3a **não roda de jeito nenhum**. Somados, os três dão o pior desfecho da skill: uma tabela bem formatada, com um punhado das entregas, que o perito só descobre errada com a ficha na mão, na diligência. Caso real (26/08, instalação nova no PC do perito): o `nlm` não subiu, a sessão caiu na mão, os 4 PDFs foram para um notebook só e a 3a estourou o limite.
> **O que fazer:** `nlm --version` (≥ 0.9.4) → `python -m pip install --user --upgrade notebooklm-mcp-cli` → `nlm login` **na conta do perito** → reiniciar o Claude Code → re-rodar o Passo 1B. Apague o efêmero que ficou de pé e **descarte o bundle** daquela rodada.

**Só quando NÃO há ficha em arquivo separado** (ela vem embutida na contestação — aí não há o que isolar e a 3a rodaria no lote de qualquer jeito), o caminho na mão é aceitável, uma pasta por vez: `notebook_create(title="EFÊMERO — <pasta>")` → para cada PDF `source_add(source_type="file", file_path="<Windows>", wait=True, wait_timeout=300)` → `notebook_get` conferindo que **todas** as fontes subiram → **Passo 2/3** (as 5 queries) → **Passo 5** (apagar) → mova a subpasta para `Processados/`.
- ⚠ **Limite de ~4,8k chars por query**: **não** cole REGRAS+P1 juntos (estoura → `INVALID_ARGUMENT`). Rode a **P1 sozinha** (cabe) e cada Parte encadeada no mesmo `conversation_id`; se quiser as REGRAS, mande-as como um turno de priming separado.
- ⚠ A 3a por chat **tem teto**: confira o total de entregas contra a ficha antes de seguir para o Passo 4, e registre a conferência para o perito.
