...
- **Recomendado — coluna `vida_util_meses` da `caepi.sqlite`** (o agente/validade já estão lá; o `build_caepi_index.py` **preserva** esses valores no rebuild). A coluna já existe; gravar via módulo `sqlite3` do Python (o CLI `sqlite3` não existe no Windows; o módulo da stdlib existe em toda plataforma):
  `python3 -c 'import sqlite3; c=sqlite3.connect(r"<base>/04-EPIs/caepi.sqlite"); c.execute("UPDATE ca SET vida_util_meses=N WHERE ca=?", ("XXXX",)); c.commit()'`
  (se a coluna ainda não existir numa base antiga, antes rodar o mesmo one-liner com `c.execute("ALTER TABLE ca ADD COLUMN vida_util_meses INTEGER")`).
