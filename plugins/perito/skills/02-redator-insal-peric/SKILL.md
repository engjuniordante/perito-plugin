...
- **Tabela de EPI (Item 5) = RESUMO POR AGENTE** (formato do Irineu, **não** a ficha bruta): colunas `DESCRIÇÃO · AGENTE · C.A. · <ano> · <ano> · <ano>`. Uma linha por **tipo de EPI + CA ligado a um agente analisado** (protetor→Ruído, capa→Umidade, creme→Químico, máscara/PFF→Poeira, capuz→RNI…), com a **quantidade entregue por ano** nas colunas de ano. **Incluir só os EPIs que neutralizam/atenuam agente** (excluir luva mecânica, botina, perneira, colete, óculos comuns, calça/blusão genéricos). ⚠ **Vestimenta impermeável (capa, calça, blusão) protege UMIDADE (An.10) — NUNCA reclassificar como "Químico".** Se Umidade está `[Ausente]` no formulário, esses itens **não entram na tabela** (não há agente analisado a vincular — luva/creme é que protegem químico cutâneo, não capa de chuva). As colunas de ano = os anos do **imprescrito** com entrega relevante (marcadores `{{EPI_ANO_1..3}}` no cabeçalho).
  > **Fonte e conversão (regra-chave do projeto):** o **formulário de campo traz a ficha COMPLETA de entregas** (todas as linhas — Data·Qtd·Descrição·CA). É o **Redator** que faz a conversão para o padrão Irineu: lê a ficha completa, **filtra** os EPIs ligados a agente, **agrupa por C.A.+agente** e **conta a quantidade por ano** (das datas da ficha). ⚠ **AGRUPE PELO C.A., NUNCA PELA GRAFIA (D3).** O manuscrito escreve o mesmo item de vários jeitos ("Luva Neoprene G", "LUVA NEOPRENE (G)", "Sental Neoprem G"); agrupar por descrição devolve uma linha por grafia — num caso real, **109 linhas para 19 C.A. distintos**. O bloco `🔧 Classificado pelo C.A.` do formulário **já vem agrupado assim** pelo `check_epi.py`, e a linha traz "(+N grafias na ficha)" quando houve variação — use-o como o mapa C.A.→agente e escolha **uma** descrição canônica (a mais legível) para o laudo. Mesmo C.A. com **duas classificações** continua em **duas linhas** — isso é informação real, não duplicata. Item **sem C.A.** agrupa pela descrição. A ficha bruta **não** vai ao laudo — fica no formulário. Isso vale para **todos os laudos** de insal/peric.
  > **⛔ REGRA DURA — classificação do EPI por agente (anti-erro, vale p/ todo EPI):** (1) **O C.A. é a CHAVE** — o `build_laudo.py` faz lookup do C.A. no `CA-dicionario.json` e **sobrepõe** a classificação; nunca classifique pelo **nome comercial/marca** ("G3", "Luz Negra" **não** é UV/radiação). (2) **Creme protetivo / de proteção = SEMPRE químico dérmico (An.13)**; única exceção é **"protetor solar"** (esse é solar/UV). (3) Sem função explícita → agente provisório, **o script resolve pelo C.A.** ou o perito confirma. (4) **DIREÇÃO PROIBIDA = EXCLUSÃO:** uma classificação **nunca** afasta/elide agente — errar incluindo é tolerável, errar excluindo corrompe o laudo.
...
- **Divergências Fáticas — item 3.1 (`{{DIVERGENCIAS_FATICAS}}`, bloco):** o campo **▶ DIVERGÊNCIAS FÁTICAS** do formulário. **Só** emitir `blocks.DIVERGENCIAS_FATICAS` (uma linha por divergência) **quando o perito preencheu** o campo — divergência real entre o alegado pelo(a) Reclamante e o constatado/o que a Reclamada afirma. **Campo em branco → NÃO enviar o marcador:** o `build_laudo.py` preenche sozinho o texto-padrão *"Durante a diligência pericial não houve divergência fática, sendo as atividades do(a) Reclamante confirmadas pela Reclamada."* Nunca inventar divergência; transcrever a substância do campo (fato, não fala crua).
- **Banheiros de grande circulação — item 3 (chave `banheiro`, tabela):** SÓ quando o formulário trouxer o bloco **▶ BANHEIROS DE GRANDE CIRCULAÇÃO** preenchido (caso de higienização de sanitários coletivos → Anexo 14 biológicos, grau máximo). Emitir no JSON:
  ```json
  "banheiro": {
    "locais": [{"local":"Bloco A - Refeitório","banheiros":4,"vasos":10}, {"local":"Vestiário","banheiros":2,"vasos":6}],
    "rotatividade": {"Funcionários":350, "Pacientes":80}
  }
  ```
...
> 🔒 **Ao varrer, vá montando `agentes_presentes` (Passo 5.1):** cada agente que você marcar aqui como `[Presente]` entra nessa lista **e** ganha um bloco `ANALISE_*` em `blocks`. O script confere um contra o outro e **recusa gerar** se você listar um Presente e esquecer de escrever a análise dele (senão ele sairia descaracterizado em silêncio).
...
- **EPI por eficácia/regularidade/período:** EPI só neutraliza se eficaz, regular e cobrindo o período. **Recorte = quantidade × vida útil por CA** (creme ≥1/mês excetua só os meses cobertos — **não é tese universal**, vale quando o EPI é insuficiente). **EPI não neutraliza periculosidade.**
  > **⚖ A RÉGUA DA COBERTURA É A DO PERITO — não recalcule, e não narre outra (D1/D2, 21/08/2026).** O número vem do bloco `📐` do `check_epi.py`, e ele agora segue duas decisões que **mudam veredicto**:
  > • **D1 — consumível de consumo contínuo EMPILHA saldo não vencido:** o pote de creme que chega antes de o anterior acabar **não perde o saldo** — **7 potes = 7 meses** (antes dava 5,6). **O protetor auricular NÃO empilha**: é de uso único com reposição por substituição (NR-6 6.6.1 "e"), então o intervalo entre entregas **é o próprio sinal** de período sem reposição; empilhar apagaria gap real (num caso medido, um gap de 55 dias a 95,6 dB(A)).
  > • **D2 — afastamento NÃO desconta da cobertura:** o denominador é o **imprescrito cheio**. Os períodos de afastamento continuam **lidos e ecoados** no `📐` (linha "afastamentos lidos, NÃO descontados"), para o perito abater à mão se quiser — **não os subtraia na análise** nem escreva "exposição = imprescrito − afastamento".
  > Se o formulário vier de uma extração **anterior a 21/08/2026**, o `📐` dele foi calculado pela régua velha (descontava afastamento, descartava saldo de creme) — **rode o `check_epi.py` de novo** antes de usar o número.
...
- ⛔ **NUNCA flexionar por gênero — sempre `O(A) Reclamante ... exposto(a)`.** Mesmo sabendo que o(a) reclamante é mulher ou homem, a ementa e a conclusão usam **só a forma neutra**: `O(A) Reclamante`, `pelo(a) Reclamante`, `exposto(a)`, `submetido(a)`. Escrever *"A Reclamante ficava exposta"* destoa da negativa que o script acrescenta logo abaixo (*"O(A) Reclamante não exerceu atividades perigosas…"*), e o laudo sai com as duas formas misturadas. **Exceção:** o **nome da função** vai como está no processo (*"Controladora de Acesso"*) — é cargo, não concordância; e **`a Reclamada`** é a empresa, sempre feminino. O `build_laudo.py` normaliza isso no bloco `CONCLUSAO_ITENS` e avisa no relatório, mas **não conte com o conserto** — escreva neutro de primeira.
...
- ⚙️ **Negativa do veredicto que falta — o SCRIPT faz (não o modelo).** No laudo `insal-peric`, quando UM dos dois (insalubridade/periculosidade) é caracterizado e o OUTRO não, o `build_laudo.py` acrescenta ao FIM da ementa E da conclusão final a frase-padrão da negativa que falta (ex.: insalubridade caracterizada → entra *"O(A) Reclamante não exerceu atividades ou operações perigosas, sendo descaracterizada a periculosidade, conforme regulamenta a NR-16…"*). Ele **lê o texto da própria conclusão** (não recalcula) e **não mexe** no que você escreveu. Você continua listando só os caracterizados em `CONCLUSAO_ITENS`; se ambos forem caracterizados, liste os dois normalmente (o script não acrescenta nada); se nada for caracterizado, use a negativa-padrão do Irineu como sempre.
...
1. **Montar o JSON `laudo-data.json`** com tudo decidido nos Passos 1–4 — **você já tem tudo em contexto, NÃO releia formulário, mapa nem `[agente].md`**. Schema:
   ...
   - `agentes_presentes` **(obrigatório)**: lista dos marcadores `ANALISE_*` de **todos** os agentes marcados `[Presente]` no formulário (inclui os abaixo do LT / descaracterizados por medição). Ex.: `["ANALISE_RUIDO_CONTINUO","ANALISE_PERIC_INFLAMAVEIS"]`. **O script confere que você emitiu um bloco em `blocks` para cada um** — se esquecer um Presente, ele seria descaracterizado em silêncio → o script **recusa gerar** (exit 2). É a rede de segurança contra omissão; monte a partir dos `[Presente]` do formulário.
   ...
   **Exemplo concreto (forma exata — use como molde, não precisa de mais nada):**
   ```json
   {
     ...
     "agentes_presentes": ["ANALISE_RUIDO_CONTINUO", "ANALISE_QUIM_QUALITATIVOS", "ANALISE_VIBRACOES"],
     "scalars": {
...
2. **Rodar o script** (monta o .docx inteiro) — caixa-preta, não leia o código. **Antes de rodar, faça a conferência do Passo 6 no seu JSON** (é a sua única chance de conferir conteúdo — depois do script não se reabre o documento):
   `python3 <base-dir>/scripts/build_laudo.py "<00-Template/{TEMPLATE DO PASSO 0}>" laudo-data.json "<config.caminhos.saida_laudos>/laudo-<processo>.docx" "<base_conhecimento>" --form <formulário preenchido>`
   - ⛔ **`--form` é OBRIGATÓRIO** (o formulário preenchido que você leu no Passo 1). Ele liga os gates 1.7/1.8: o JSON **não pode contrariar um checkbox que o perito marcou**. Se o formulário diz `Neutralizado pelo EPI: [X] Sim` num agente, o bloco `ANALISE_*` daquele agente **não pode caracterizar insalubridade**; e as linhas do quadro **NR-6 que o perito marcou** não podem sair invertidas no `nr6` do JSON. Divergiu → o script **recusa gerar (exit 2)** e você **pergunta ao perito** — não conserta sozinho. Linha da NR-6 que o perito deixou **em branco** (👤 adequação / fiscalização) você pode preencher; marcada, não.
   - 🧭 **Por quê:** texto-padrão, laudo-base e caso anterior dão a **moldura**; o formulário dá o **fato**. Um "NÃO" na NR-6 derruba a neutralização do EPI e vira o veredito — marcar essa célula é preencher um campo, não é decidir a caracterização do agente. (Origem: um laudo real saiu correto e foi invertido depois, por uma regra de outro caso.)
   ...
   - **Template (1º arg) e base EPI (4º arg) têm FALLBACK BUNDLED automático.** No Cowork o **bash não enxerga o Drive** → o script cai sozinho na cópia **bundled do MESMO template** em `assets/templates/` (os **três** estão bundled na schema atual) e na base EPI bundled da skill 01 (imprime `ℹ️ usando o BUNDLED`). O fallback **nunca troca de tipo**: falta o bundled do tipo pedido → falha em vez de substituir. Passe o caminho do Drive normalmente; nativo usa o vivo, Cowork usa o bundled. **Nunca redija o .docx à mão nem pule o script porque "o template não está acessível".**
   ...
   - **SAÍDA = `config.caminhos.saida_laudos`** (no Irineu: `Base Perícia Irineu/Laudos-Gerados/`) — o **mesmo destino da 03 e da 04/04b**, para laudo e esclarecimentos não se espalharem. O script **cria a pasta** se ela não existir.
