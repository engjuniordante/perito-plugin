...
2. **Rodar o script** (monta o .docx inteiro) — caixa-preta, não leia o código. **Antes de rodar, faça a conferência do Passo 6 no seu JSON** (é a sua única chance de conferir conteúdo — depois do script não se reabre o documento):
   `python3 <base-dir>/scripts/build_laudo.py <00-Template/template-insal-peric.docx> laudo-data.json <SAÍDA> <base_conhecimento>/04-EPIs/CA-dicionario.json`
   - O 4º argumento (CA-dicionario.json, caminho da base pelo `perito-config.json`) faz o script **classificar a tabela de EPI pelo C.A.** (chave primária; ignora o nome comercial), auto-corrigir creme→An.13 e listar C.A. não catalogados. Omitir o arg = sem lookup (só a regra absoluta).
